...
export default function Tracker() {
  ...
  const addClient = async (data: Partial<Client>) => {
    const res = await supabase.from('clients').insert(data).select()
    loadData()
    if (res.data && res.data[0]) setSelId(res.data[0].id)
  }
    ...
    <div style={{ maxWidth: 1060, margin: '0 auto', padding: '20px 16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => { setShowAddClient(!showAddClient); setShowAging(false) }} style={{ padding: '5px 12px', borderRadius: 6, fontSize: 10, fontWeight: 600, cursor: 'pointer', border: '1px solid rgba(34,197,94,0.3)', background: showAddClient ? '#22c55e' : 'rgba(34,197,94,0.08)', color: showAddClient ? '#fff' : '#22c55e' }}>
            + Cliente
          </button>
          <button onClick={() => { setShowAging(!showAging); setShowAddClient(false) }} style={{ padding: '5px 12px', borderRadius: 6, fontSize: 10, fontWeight: 600, cursor: 'pointer', border: '1px solid rgba(255,255,255,0.08)', background: showAging ? '#6366f1' : 'transparent', color: showAging ? '#fff' : '#6b7185' }}>
            Aging Report
          </button>
        </div>
      </div>
      ...
      {showAddClient && (
        <AddClientForm
          onAdd={(data) => { addClient(data); setShowAddClient(false) }}
...
function ClientPanel({
  ...
  onAddNote, onEditNote, onDeleteNote, onAddInvoice, onUpdateClient,
}: {
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(560px, 93vw)', background: '#0c0e14', borderLeft: '1px solid rgba(255,255,255,0.06)', zIndex: 100, overflowY: 'auto', boxShadow: '-10px 0 40px rgba(0,0,0,0.4)' }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: '#0c0e14', zIndex: 2 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={() => setShowEditClient(!showEditClient)} style={{ background: '#12151e', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, color: showEditClient ? '#6366f1' : '#6b7185', padding: '5px 10px', cursor: 'pointer', fontSize: 10, fontWeight: 600 }}>
              Ficha
            </button>
        ...
        {showEditClient && (
          <EditClientForm
            client={client}
            onSave={(data) => { onUpdateClient(client.id, data); setShowEditClient(false) }}
...
// ─── Add Client Form ───
function AddClientForm({ onAdd, onCancel }: { onAdd: (data: Partial<Client>) => void; onCancel: () => void }) {
  const [name, setName] = useState('')
  const [legalName, setLegalName] = useState('')
  const [currency, setCurrency] = useState('EUR')
  const [method, setMethod] = useState('')
  const [reqInv, setReqInv] = useState(false)
  const [address, setAddress] = useState('')
  const [comments, setComments] = useState('')
  const [wallet, setWallet] = useState('')
  const [ourWallet, setOurWallet] = useState('')
    ...
    <div style={{ marginBottom: 16, padding: '18px 20px', borderRadius: 12, background: '#12151e', border: '1px solid rgba(34,197,94,0.2)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        <div>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Ex: ChilliPartners" autoFocus style={iBase} />
        </div>
        <div>
          <label style={lbl}>Nome Legal</label>
          <input value={legalName} onChange={e => setLegalName(e.target.value)} placeholder="Ex: Scartesu Limited" style={iBase} />
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {['EUR', 'USD'].map(c => (
              <button key={c} onClick={() => setCurrency(c)} style={{ flex: 1, padding: '8px', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: '1px solid rgba(255,255,255,0.08)', background: currency === c ? '#6366f1' : 'transparent', color: currency === c ? '#fff' : '#6b7185' }}>{c}</button>
            ))}
          </div>
        </div>
        <div>
          <label style={lbl}>Método Pagamento</label>
          <select value={method} onChange={e => setMethod(e.target.value)} style={{ ...iBase, appearance: 'none' as const }}>
        ...
        <div>
          ...
          <input value={threshold} onChange={e => setThreshold(e.target.value)} type="number" step="0.01" placeholder="Mínimo" style={{ ...iBase, fontFamily: "'DM Mono', monospace" }} />
        </div>
      </div>
      <div style={{ marginBottom: 10 }}>
        <label style={lbl}>Morada / Info Legal</label>
        <input value={address} onChange={e => setAddress(e.target.value)} placeholder="Morada, VAT, reg. number..." style={iBase} />
      </div>
      <div style={{ marginBottom: 10 }}>
        <label style={lbl}>Comentários / Instruções</label>
        <input value={comments} onChange={e => setComments(e.target.value)} placeholder="Links, instruções..." style={iBase} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        <div>
          <label style={lbl}>Wallet Crypto (cliente)</label>
          <input value={wallet} onChange={e => setWallet(e.target.value)} placeholder="TRC20..." style={{ ...iBase, fontFamily: "'DM Mono', monospace", fontSize: 11 }} />
        </div>
        <div>
          <label style={lbl}>Nossa Wallet USDT</label>
          <input value={ourWallet} onChange={e => setOurWallet(e.target.value)} placeholder="TRC20..." style={{ ...iBase, fontFamily: "'DM Mono', monospace", fontSize: 11 }} />
        </div>
      </div>
      <div style={{ marginBottom: 12 }}>
        <label style={lbl}>Info Adicional</label>
        <input value={extra} onChange={e => setExtra(e.target.value)} placeholder="Dados extra..." style={iBase} />
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        <input type="checkbox" checked={reqInv} onChange={e => setReqInv(e.target.checked)} id="reqInv" style={{ accentColor: '#6366f1' }} />
        <label htmlFor="reqInv" style={{ fontSize: 12, color: '#e8eaf0', cursor: 'pointer' }}>Requer fatura</label>
      </div>
      <button onClick={() => {
        if (!name.trim()) return
        onAdd({ name: name.trim(), legal_name: legalName || null, currency, payment_method: method || null, requires_invoice: reqInv, address: address || null, comments: comments || null, wallet_crypto: wallet || null, our_wallet: ourWallet || null, extra_info: extra || null, threshold: threshold ? parseFloat(threshold) : null, is_active: true })
      }} style={{ width: '100%', padding: 12, borderRadius: 8, border: 'none', background: '#22c55e', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
        Criar Cliente
      </button>
    </div>
  )
...
// ─── Edit Client Form ───
function EditClientForm({ client, onSave, onCancel }: { client: Client; onSave: (data: Partial<Client>) => void; onCancel: () => void }) {
  const [name, setName] = useState(client.name)
  const [legalName, setLegalName] = useState(client.legal_name || '')
  const [currency, setCurrency] = useState(client.currency)
  const [method, setMethod] = useState(client.payment_method || '')
  const [reqInv, setReqInv] = useState(client.requires_invoice)
  const [address, setAddress] = useState(client.address || '')
  const [comments, setComments] = useState(client.comments || '')
  const [wallet, setWallet] = useState(client.wallet_crypto || '')
  const [ourWallet, setOurWallet] = useState(client.our_wallet || '')
  const [extra, setExtra] = useState(client.extra_info || '')
  const [threshold, setThreshold] = useState(client.threshold ? String(client.threshold) : '')
  ...
  return (
    <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        <div>
          <label style={lbl}>Nome Comercial</label>
          <input value={name} onChange={e => setName(e.target.value)} style={iBase} />
        </div>
        <div>
          <label style={lbl}>Nome Legal</label>
          <input value={legalName} onChange={e => setLegalName(e.target.value)} style={iBase} />
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
        <div>
        ...
        <div>
          <label style={lbl}>Threshold</label>
          <input value={threshold} onChange={e => setThreshold(e.target.value)} type="number" step="0.01" style={{ ...iBase, fontFamily: "'DM Mono', monospace" }} />
        </div>
      </div>
      <div style={{ marginBottom: 10 }}>
        <label style={lbl}>Morada / Info Legal</label>
        <input value={address} onChange={e => setAddress(e.target.value)} style={iBase} />
      </div>
      <div style={{ marginBottom: 10 }}>
        <label style={lbl}>Comentários</label>
        <textarea value={comments} onChange={e => setComments(e.target.value)} rows={2} style={{ ...iBase, resize: 'vertical' as const }} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        <div>
          <label style={lbl}>Wallet Crypto (cliente)</label>
          <input value={wallet} onChange={e => setWallet(e.target.value)} style={{ ...iBase, fontFamily: "'DM Mono', monospace", fontSize: 11 }} />
        </div>
        <div>
          <label style={lbl}>Nossa Wallet</label>
          <input value={ourWallet} onChange={e => setOurWallet(e.target.value)} style={{ ...iBase, fontFamily: "'DM Mono', monospace", fontSize: 11 }} />
        </div>
      </div>
      <div style={{ marginBottom: 10 }}>
        <label style={lbl}>Info Adicional</label>
        <input value={extra} onChange={e => setExtra(e.target.value)} style={iBase} />
      </div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <input type="checkbox" checked={reqInv} onChange={e => setReqInv(e.target.checked)} id="editReqInv" style={{ accentColor: '#6366f1' }} />
          <label htmlFor="editReqInv" style={{ fontSize: 11, color: '#e8eaf0', cursor: 'pointer' }}>Requer fatura</label>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <input type="checkbox" checked={active} onChange={e => setActive(e.target.checked)} id="editActive" style={{ accentColor: '#22c55e' }} />
          <label htmlFor="editActive" style={{ fontSize: 11, color: '#e8eaf0', cursor: 'pointer' }}>Ativo</label>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button onClick={() => {
          if (!name.trim()) return
          onSave({ name: name.trim(), legal_name: legalName || null, currency, payment_method: method || null, requires_invoice: reqInv, address: address || null, comments: comments || null, wallet_crypto: wallet || null, our_wallet: ourWallet || null, extra_info: extra || null, threshold: threshold ? parseFloat(threshold) : null, is_active: active })
